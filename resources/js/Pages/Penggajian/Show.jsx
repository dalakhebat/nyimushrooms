    ...
    return (
        <AdminLayout title="Detail Slip Gaji">
                    ...
                    {penggajian.status === 'pending' && (
                        <>
                            <Link
                                ...
                                className="inline-flex items-center px-4 py-2 bg-yellow-500 text-white text-sm font-medium rounded-xl hover:bg-yellow-600"
                            >
            ...
            <div className="bg-surface-container-lowest rounded-xl shadow-clinical-sm overflow-hidden">
                ...
                <div className="bg-primary text-white p-6">
                    <div className="flex items-center justify-between">
                        ...
                        <div className="text-right">
                            ...
                            <p className="text-lg font-bold">{penggajian.periode_formatted}</p>
                        </div>
                    </div>
                </div>

                {/* Status Badge */}
                <div className="px-6 py-3 bg-surface-container-low border-b flex items-center justify-between">
                    ...
                    <div className="text-xs text-on-tertiary-container text-right">
                ...
                <div className="p-6">
                    {/* Karyawan Info */}
                    <div className="flex items-start mb-6 pb-6 border-b border-outline-variant/15">
                        <div className="bg-emerald-100 p-3 rounded-full mr-4">
                            <UserIcon className="w-8 h-8 text-secondary" />
                        </div>
                        <div>
                            <h2 className="text-xl font-bold text-on-surface">{penggajian.karyawan.nama}</h2>
                            ...
                            {penggajian.karyawan.no_hp && (
                                <p className="text-sm text-on-tertiary-container mt-1">{penggajian.karyawan.no_hp}</p>
                            )}
                        </div>
                    </div>
                    ...
                    <div className="mb-6">
                        <div className="flex items-center justify-between mb-3">
                            <h3 className="text-sm font-medium text-on-tertiary-container uppercase">Rekap Kehadiran</h3>
                            {penggajian.status === 'pending' && (
                                <button
                        ...
                        <div className="grid grid-cols-4 gap-4">
                            <div className="bg-emerald-50 p-4 rounded-xl">
                                <div className="text-center mb-2">
                                    <p className="text-2xl font-bold text-secondary">{penggajian.jumlah_hadir}</p>
                                    <p className="text-xs text-secondary">Hadir</p>
                                </div>
                                {penggajian.detail_kehadiran?.hadir?.length > 0 && (
                                    <div className="text-xs text-secondary border-t border-green-200 pt-2 mt-2">
                    ...
                    {/* Rincian Gaji */}
                    <div className="mb-6">
                        <h3 className="text-sm font-medium text-on-tertiary-container uppercase mb-3">Rincian Gaji</h3>
                        <div className="bg-surface-container-low rounded-xl p-4 space-y-3">
                            <div className="flex justify-between">
                                <span className="text-on-surface-variant">Gaji Pokok</span>
                                <span className="font-medium text-on-surface">{formatCurrency(penggajian.gaji_pokok)}</span>
                            </div>
                            {penggajian.bonus > 0 && (
                            ...
                            <div className="border-t border-outline-variant/30 pt-3 flex justify-between">
                                <span className="font-bold text-on-surface">Total Diterima</span>
                                <span className="font-bold text-xl text-secondary">{formatCurrency(penggajian.total)}</span>
                            </div>
                        </div>
                    </div>
                    ...
                    {penggajian.catatan && (
                        <div>
                            <h3 className="text-sm font-medium text-on-tertiary-container uppercase mb-2">Catatan</h3>
                            <p className="text-on-surface-variant bg-surface-container-low p-3 rounded-xl">{penggajian.catatan}</p>
                        </div>
                    )}
                </div>
            </div>
            ...
            {showKoreksiModal && (
                <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
                    <div className="bg-surface-container-lowest rounded-xl p-6 w-full max-w-lg">
                        <h3 className="text-lg font-bold text-on-surface mb-4">Koreksi Absensi</h3>
                            ...
                            <table className="w-full text-sm">
                                ...
                                <tbody className="divide-y divide-gray-200">
                                    {allAbsensi.map((absensi) => (
                                        <tr
                                            key={absensi.id}
                                            className={`hover:bg-surface-container-low ${selectedAbsensi?.id === absensi.id ? 'bg-orange-50' : ''}`}
                                        >
                                            <td className="px-3 py-2">{absensi.tanggal}</td>
                                            <td className="px-3 py-2">
                                                <span className={`inline-flex px-2 py-0.5 text-xs font-medium rounded-full ${
                                                    absensi.status === 'hadir' ? 'bg-emerald-100 text-secondary' :
                                        ...
                                        <button
                                            ...
                                            className={`px-3 py-1 text-sm rounded-full border transition-colors ${
                                                koreksiStatus === status
                                                    ? status === 'hadir' ? 'bg-primary text-white border-green-600' :
                        ...
                        <div className="flex justify-end space-x-3">
                            ...
                            <button
                                ...
                                className="px-4 py-2 bg-orange-600 text-white rounded-xl hover:bg-orange-700 disabled:bg-gray-300 disabled:cursor-not-allowed"
                            >
                                Simpan Koreksi
                            </button>
                        </div>

                        <p className="text-xs text-on-tertiary-container mt-4">
